...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           skip_download;             /* Skip download firmware?          */
...
const char *config_file = "jlink.conf";
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  // if (retry > 0) {
  //   status = trace_res > 0 ? AFL_RES_CRASH : status;
  // } else {
  //   status = AFL_RES_TIMEOUT;
  // }

  // if (status == AFL_RES_TIMEOUT) {
  //   child_timed_out = 1;
  // }

  if (retry == 0 || status == TARGET_TIMEOUT) {
    child_timed_out = 1;
  }
...
int main(int argc, char** argv) {
  ...
  memset(&_Paras, 0, sizeof(_Paras));

  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:c:dnpCNB:S:M:x:Q")) > 0)
    ...
    switch (opt) {
      ...
      case 'N': /* skip download firmware */

        skip_download = 1;
        break;

      case 'p': /* persistent mode */
  ...
  if (argc - optind < 1 || !in_dir || !out_dir) usage(argv[0]);

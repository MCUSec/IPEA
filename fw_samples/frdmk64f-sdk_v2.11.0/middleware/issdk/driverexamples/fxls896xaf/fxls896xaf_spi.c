...
const registerreadlist_t cFxls8962DRDYEvent[] = {{.readFrom = FXLS896x_INT_STATUS, .numBytes = 1}, __END_READ_DATA__};

/*! @brief Address of Raw Accel Data in Normal Mode. */
const registerreadlist_t cFxls8962OutputNormal[] = {{.readFrom = FXLS896x_OUT_X_LSB, .numBytes = FXLS896x_DATA_SIZE},
                                                    __END_READ_DATA__};
...
int main(void)
{
    ...
    else if (FXLS896x_WHOAMI_VALUE == whoami)
    {
    ...
    status = FXLS896x_SPI_Configure(&fxls896xDriver, cFxls8962ConfigNormal);
    if (SENSOR_ERROR_NONE != status)
    {
    ...
    for (;;) /* Forever loop */
    {
        ...
        status = FXLS896x_SPI_ReadData(&fxls896xDriver, cFxls8962DRDYEvent, &gFxls8962DataReady);
        if (0 == (gFxls8962DataReady & FXLS896x_INT_STATUS_SRC_DRDY_MASK))
        { /* Loop, if new sample is not available. */
            continue;
        }

        /*! Read the raw sensor data from the FXLS896x. */
        status = FXLS896x_SPI_ReadData(&fxls896xDriver, cFxls8962OutputNormal, data);
        if (ARM_DRIVER_OK != status)
        {
            PRINTF("\r\nRead Failed.\r\n");
            return -1;

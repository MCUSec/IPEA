...
typedef struct {
  SYS_TIME CompileDate;
  U8 IsInvalid;
  U8 IsBTL;
} UTIL_JLINK_FW_VERSION_INFO;
...
void            UTIL_StoreBytesIncDest        (U8** ppDest, void* pSrc, U32 NumBytes);
...
void            UTIL_Store64LEIncPtr          (U8** pp, U64 Data);
...
int             UTIL_GetJLinkFWVersionInfo    (UTIL_JLINK_FW_VERSION_INFO* pInfo, const char* sFirmware);
int             UTIL_CompareDates             (const SYS_TIME* pTimeSrc, const SYS_TIME* pTimeDest);
int             UTIL_GetTimeDiff              (const SYS_TIME* pLeft, const SYS_TIME* pRight);

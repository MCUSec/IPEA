...
//
#define JLINK_MEM_ACC_TYPE_DEFAULT          (0)  // Generic. Actual access type depends on connected core. E.g. for Cortex-A default is accessing memory through the CPU (feeding memory instructions to it). For Cortex-M default is access via AHB-AP (DMA-like)
#define JLINK_MEM_ACC_TYPE_0                (0)
#define JLINK_MEM_ACC_TYPE_CPU              (1)  // Generic. Memory is accessed through the CPU by feeding memory instructions to it. Not supported for all cores.
...
//
// Cortex specific access types
//
//                                                  0-1 are identical for all cores
#define JLINK_MEM_ACC_TYPE_CORTEX_PRECONFIG_AHBAP   (JLINK_MEM_ACC_TYPE_2)
#define JLINK_MEM_ACC_TYPE_CORTEX_PRECONFIG_APBAP   (JLINK_MEM_ACC_TYPE_3)
#define JLINK_MEM_ACC_TYPE_CORTEX_MEMAP             (JLINK_MEM_ACC_TYPE_4)   // Access via MEM-AP. Index of AP to be used is specified by Flags[20:13]
//
// RISC-V specific access types
//
//                                                  0-1 are identical for all cores
#define JLINK_MEM_ACC_TYPE_RISCV_CPU               (JLINK_MEM_ACC_TYPE_1)
#define JLINK_MEM_ACC_TYPE_RISCV_SBA               (JLINK_MEM_ACC_TYPE_2)
#define JLINK_MEM_ACC_TYPE_RISCV_MEMAP             (JLINK_MEM_ACC_TYPE_3)   // Access via MEM-AP. Index of AP to be used is specified by Flags[20:13]
//
// Access width
//
...
#define JLINK_EMU_PRODUCT_ID_FLASHER_ATE_MB     (0x29)
#define JLINK_EMU_PRODUCT_ID_FLASHER_HUB        (0x2A)
...
#define JLINK_CORE_CORTEX_R52       0x1404FFFF
#define JLINK_CORE_CORTEX_A55       0x1405FFFF
#define JLINK_CORE_CORTEX_A32       0x1406FFFF
//
...
#define JLINK_EMU_FILE_ATT_DIR                      (1 << 0)  // Entry describes a directory, not a file.
#define JLINK_EMU_FILE_ATT_NAME_TRIMMED             (1 << 1)  // File name was trimmed due to buffer limitations. Set by host side in case name returned by probe is longer than what host buffer can handle
...
typedef enum {
  ...
  JLINK_ARM_V8AR_REG_R31,  // Does not really exist in HW but made it into the API by accident. Will return fixed 0x0 for current versions.
...
typedef struct {
  char acFile[256];                   // File name incl. extension. Name of sub directory in case we do not have a file but directory.
  U32 FileSize;                       // File size in bytes.
  U32 Attributes;                     // See JLINK_EMU_FILE_ATT_ defines. E.g. may contain information that the entry is another directory, not a file.
  U16 Year;                           // Last modified
  U16 Month;                          // Last modified
  U16 Day;                            // Last modified
  U16 Hour;                           // Last modified
  U16 Minute;                         // Last modified
  U16 Second;                         // Last modified
} JLINK_EMU_FILE_INFO;
...
typedef enum {
  ...
  ,JLINK_FLASHDL_SPI_STAT_REG_FORMAT_MICROCHIP   = (9)   // [0:0] == Busy
  ,JLINK_FLASHDL_SPI_STAT_REG_FORMAT_ADESTO_PE   = (10)  // [0:0] == Page_Size, [1:1] == Sector Protection, [7:7] == Busy
  ,JLINK_FLASHDL_SPI_STAT_REG_FORMAT_NONE        = (11)  // StatRegFormat is unknown (e.g. in manual mode). The software does not perform any special handling such as "auto" clear block protection.
...
typedef int STDCALL JLINK_FUNC_EMU_FILE_GET_FILE_INFO(const char* sPath, JLINK_EMU_FILE_INFO* pInfo);
typedef int STDCALL JLINK_FUNC_EMU_FILE_GET_DIR_ENTRIES(const char* sPath, JLINK_EMU_FILE_INFO* paInfo, U32 MaxNumInfo);
typedef int STDCALL JLINK_FUNC_EMU_FILE_MOVE(const char* sDest, const char* sSrc);
typedef int STDCALL JLINK_FUNC_UART_INIT(U32 BaudrateHz, U32 Flags);
typedef int STDCALL JLINK_FUNC_UART_WRITE(const void* pData, U32 NumBytes);
typedef int STDCALL JLINK_FUNC_UART_READ(void* pData, U32 NumBytes, U32 Timeout);
typedef int STDCALL JLINK_FUNC_UART_DEINIT(void);
typedef int STDCALL JLINK_FUNC_PCODE_DATA_DOWNLOAD_EX(U8* pData, U32 DataOffset, U32 NumBytes);
...
#define JLINKARM_CM3_RESET_TYPE_HALT_AFTER_BTL_USING_WP ((JLINKARM_RESET_TYPE)(12))     // Details: https://wiki.segger.com/Reset_and_Halt_After_Bootloader
